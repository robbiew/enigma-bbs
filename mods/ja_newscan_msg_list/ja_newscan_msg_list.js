...
const MessageAreaConfTempSwitcher =
    require('../../core/mod_mixins.js').MessageAreaConfTempSwitcher;
...
exports.moduleInfo = {
    name: 'Newscan Message List',
    desc: 'Module for listing/browsing messages during newscan with enhanced mark-all-read functionality',
    author: 'NuSkooler (Enhanced by ja_newscan)',
    packageName: 'com.ja.newscan.msg-list',
};
...
exports.getModule = class NewscanMessageListModule extends (
    MessageAreaConfTempSwitcher(MenuModule)
) {
    constructor(options) {
        super(options);

        // Add logging to confirm module is loaded
        if (options && options.client && options.client.log) {
            options.client.log.info('NewscanMessageListModule constructor called');
        }
        ...
        this.menuMethods = {
            ...
            // ENHANCED: Custom markAllRead method for newscan
            markAllRead: (formData, extraArgs, cb) => {
                ...
                return this.markAllMessagesAsRead(() => {
                    // Show visual feedback message using proper MCI positioning
                    // Let the art file control colors via %TL99 MCI styling
                    this.setViewText('allViews', 99, '>> messages marked as read <<');

                    // Brief pause then clear message and return to newscan
                    setTimeout(() => {
                        // Clear the status message
                        this.setViewText('allViews', 99, '');
                        return this.prevMenu(cb);
    ...
    populateCustomLabelsForSelected(selectedIndex) {
        const selectedMessage = this.config.messageList[selectedIndex];
        const areaTag = selectedMessage.areaTag || this.config.messageAreaTag;

        this.client.log.info({
            selectedIndex: selectedIndex,
            areaTag: areaTag,
            messageAreaTag: this.config.messageAreaTag,
            selectedMessage: selectedMessage
        }, 'populateCustomLabelsForSelected called');

        // Get area and conference information for display
        const areaInfo = messageArea.getMessageAreaByTag(areaTag);
        const confInfo = areaInfo ? messageArea.getMessageConferenceByTag(areaInfo.confTag) : null;
        ...
        const formatObj = Object.assign(
            {
                ...
                // Add current area and conference information for custom MCI codes
                currentAreaName: areaInfo ? areaInfo.name : 'Unknown Area',
                currentConfName: confInfo ? confInfo.name : 'Unknown Conference',
                currentArea: areaInfo ? areaInfo.name : 'Unknown Area',
                currentConf: confInfo ? confInfo.name : 'Unknown Conference',
                scanAreaName: areaInfo ? areaInfo.name : 'Unknown Area',
                scanConfName: confInfo ? confInfo.name : 'Unknown Conference'
            },
            selectedMessage //  plus, all the selected message props
        );

        this.client.log.info({
            formatObj: formatObj,
            customRangeStart: MciViewIds.allViews.customRangeStart
        }, 'Calling updateCustomViewTextsWithFilter with format object');

        return this.updateCustomViewTextsWithFilter(
            'allViews',
            13, // Use MCI ID 13 to match our configuration (TL13, TL14, TL15)
            formatObj
        );
    }
